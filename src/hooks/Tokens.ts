...
import { Currency, ETHER, Token, currencyEquals, ChainId } from '@alayanetwork/uniswap-sdk'
...
export function useToken(tokenAddress?: string): Token | undefined | null {
  ...
  if (tokenAddress?.startsWith('0x')) {
    tokenAddress = toBech32Address(hrp, tokenAddress)
  }
...
export function useCurrency(currencyId: string | undefined): Currency | null | undefined {
  const isATP = currencyId?.toUpperCase() === 'ATP'
  const token = useToken(isATP ? undefined : currencyId)
  return isATP ? ETHER : token
}
